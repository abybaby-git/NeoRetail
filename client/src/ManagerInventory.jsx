...
const ManagerInventory = () => {
  ...
  const fetchStoreStock = async (storeId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/stock?store_id=${storeId}`, {
        headers: {
  ...
  const fetchLowStockItems = async (storeId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/pos/low-stock/${storeId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const checkUserStoreAssignment = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/users/me?user_id=${decodedToken.user_id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
